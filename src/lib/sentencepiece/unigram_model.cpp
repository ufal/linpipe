...
#include "lib/sentencepiece/third_party/absl/container/flat_hash_map.h"
...
void Lattice::Clear() {
  begin_nodes_.clear();
  end_nodes_.clear();
  sentence_ = linpipe::sentencepiece::absl::string_view("");
  surface_.clear();
...
Lattice::Node *Lattice::Insert(int pos, int length) {
  ...
  node->piece = linpipe::sentencepiece::absl::string_view(surface(pos), utf8_length);
...
Hypothesis *CloneHypAndDependents(
    ...
    linpipe::sentencepiece::absl::flat_hash_map<const Hypothesis *, Hypothesis *> *clone_map,
...
std::vector<Lattice::LatticePathWithScore> Lattice::NBest(size_t nbest_size,
                                                          ...
                                                          float inv_theta) {
  if (nbest_size < 1) {
    LOG(WARN, "nbest_size >= 1. Returns empty result.");
    return {};
  }
  ...
  while (!agenda.empty()) {
    ...
    if (hypothesis_allocator.size() >= kOneBillion) {
      if (!printed_memory_warning) {
        ...
        LOG(WARN, "Allocator size exceeds " << kOneBillion
    ...
    if (agenda.size() >= kMaxAgendaSize) {
      ...
      shrink_count++;
      LOG(WARN, "Too big agenda size " << agenda.size()
...
bool Model::VerifyOutputsEquivalent(absl::string_view expected,
                                    linpipe::sentencepiece::absl::string_view actual) const {
  ...
  if (std::abs(expected_score - actual_score) > kEpsilon) {
    LOG(WARN, "Two sentence piece sequences are not equivalent! Left: "
