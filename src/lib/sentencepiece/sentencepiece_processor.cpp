...
#include "lib/sentencepiece/normalizer.h"
#include "lib/sentencepiece/builtin_pb/sentencepiece.pb.h"
#include "lib/sentencepiece/third_party/absl/memory/memory.h"
#include "lib/sentencepiece/third_party/absl/strings/numbers.h"
#include "lib/sentencepiece/third_party/absl/strings/str_cat.h"
#include "lib/sentencepiece/third_party/absl/strings/str_join.h"
#include "lib/sentencepiece/third_party/absl/strings/str_replace.h"
#include "lib/sentencepiece/third_party/absl/strings/str_split.h"
#include "lib/sentencepiece/third_party/absl/strings/string_view.h"
#include "lib/sentencepiece/third_party/absl/strings/strip.h"
...
void ConvertToUnicodeSpansInternal(SentencePieceText *spt) {
  ...
  linpipe::sentencepiece::absl::string_view str = spt->text();
  size_t prev = 0;
  int ulen = 0;
  while (!str.empty()) {
...
util::Status SentencePieceProcessor::Load(absl::string_view filename) {
  auto model_proto = linpipe::sentencepiece::absl::make_unique<ModelProto>();
...
util::Status SentencePieceProcessor::Load(const ModelProto &model_proto) {
  auto model_proto_copy = linpipe::sentencepiece::absl::make_unique<ModelProto>();
...
util::Status SentencePieceProcessor::LoadFromSerializedProto(
    linpipe::sentencepiece::absl::string_view serialized) {
  auto model_proto = linpipe::sentencepiece::absl::make_unique<ModelProto>();
  CHECK_OR_RETURN(
...
util::Status SentencePieceProcessor::Load(
  ...
  normalizer_ = linpipe::sentencepiece::absl::make_unique<normalizer::Normalizer>(
  ...
  if (model_proto_->has_denormalizer_spec() &&
    ...
    denormalizer_ = linpipe::sentencepiece::absl::make_unique<normalizer::Normalizer>(
  ...
  for (const auto &s : model_proto_->self_test_data().samples()) {
    ...
    const std::string result = linpipe::sentencepiece::absl::StrJoin(sps, " ");
    if (!model_->VerifyOutputsEquivalent(s.expected(), result)) {
      errors.emplace_back(
          linpipe::sentencepiece::absl::StrCat(s.input(), "\t", s.expected(), "\t", result));
    }
  }
...
util::Status SentencePieceProcessor::SetEncodeExtraOptions(
    linpipe::sentencepiece::absl::string_view extra_options) {
...
util::Status SentencePieceProcessor::LoadVocabulary(absl::string_view filename,
  ...
  while (input->ReadLine(&line)) {
    const std::vector<std::string> v = linpipe::sentencepiece::absl::StrSplit(line, "\t");
...
util::Status SentencePieceProcessor::Encode(
    linpipe::sentencepiece::absl::string_view input, std::vector<std::string> *pieces) const {
...
util::Status SentencePieceProcessor::NBestEncode(
    linpipe::sentencepiece::absl::string_view input, int nbest_size,
...
util::Status SentencePieceProcessor::SampleEncode(
    linpipe::sentencepiece::absl::string_view input, int nbest_size, float alpha,
...
util::Status SentencePieceProcessor::SampleEncodeAndScore(
    linpipe::sentencepiece::absl::string_view input, int num_samples, float alpha, bool wor,
    bool include_best,
...
util::Status SentencePieceProcessor::PopulateSentencePieceText(
    linpipe::sentencepiece::absl::string_view input, linpipe::sentencepiece::absl::string_view normalized,
    const std::vector<size_t> &norm_to_orig, const EncodeResult &result,
    SentencePieceText *spt) const {
  size_t consumed = 0;
  bool is_prev_unk = false;
  for (const auto &p : result) {
    const linpipe::sentencepiece::absl::string_view w = p.first;  // piece
    ...
    if (IsControl(id)) {
      ...
      const auto surface =
          linpipe::sentencepiece::absl::ClippedSubstr(input, orig_begin, orig_end - orig_begin);
...
util::Status SentencePieceProcessor::SampleEncodeAndScore(
    linpipe::sentencepiece::absl::string_view input, int samples, float alpha, bool wor,
...
util::Status SentencePieceProcessor::Decode(
  ...
  auto DecodeSentencePiece =
    ...
    if (is_bos_ws &&
      ...
      has_bos_ws = linpipe::sentencepiece::absl::ConsumePrefix(&piece, kSpaceSymbol);
  ...
  auto SetSurface = [&](int index, linpipe::sentencepiece::absl::string_view surface) {
    auto *sp = spt->mutable_pieces(index);
    sp->set_surface(std::string(surface));
    sp->set_begin(text->size());
    sp->set_end(text->size() + surface.size());
    linpipe::sentencepiece::absl::StrAppend(text, surface);
  };
  ...
  auto ProcessBytePieces = [&](int token_index_begin,
    ...
    while (offset < bytes_len) {
      ...
      const bool is_valid = string_util::IsValidDecodeUTF8(
          linpipe::sentencepiece::absl::string_view(bytes).substr(offset), &consumed);
      ...
      if (!is_valid) {
        ...
        const linpipe::sentencepiece::absl::string_view utf8 =
            linpipe::sentencepiece::absl::string_view(bytes).substr(offset, consumed);
  ...
  for (const int id : ids) {
    if (id < 0 || id >= num_pieces) {
      return util::Status(util::StatusCode::kOutOfRange,
                          linpipe::sentencepiece::absl::StrCat("Invalid id: ", id));
    }
...
util::Status SentencePieceProcessor::ParseExtraOptions(
    linpipe::sentencepiece::absl::string_view _extra_option,
    std::vector<SentencePieceProcessor::ExtraOption> *extra_options) const {
  linpipe::sentencepiece::absl::string_view extra_option(_extra_option.data(), _extra_option.size());
  ...
  for (const auto &s : linpipe::sentencepiece::absl::StrSplit(extra_option, ":")) {
