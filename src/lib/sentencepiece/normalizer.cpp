...
#include "lib/sentencepiece/third_party/absl/strings/match.h"
...
#include "lib/sentencepiece/third_party/darts_clone/darts.h"
...
void Normalizer::Init() {
  linpipe::sentencepiece::absl::string_view index = spec_->precompiled_charsmap();
  if (!index.empty()) {
    linpipe::sentencepiece::absl::string_view trie_blob, normalized;
#ifdef IS_BIG_ENDIAN
    ...
    trie_ = linpipe::sentencepiece::absl::make_unique<Darts::DoubleArray>();
...
util::Status Normalizer::Normalize(absl::string_view input,
  ...
  const linpipe::sentencepiece::absl::string_view kSpaceSymbol = "\xe2\x96\x81";
  ...
  while (!input.empty()) {
    ...
    linpipe::sentencepiece::absl::string_view sp = p.first;

    // Removes heading spaces in sentence piece,
    // if the previous sentence piece ends with whitespace.
    while (is_prev_space && linpipe::sentencepiece::absl::ConsumePrefix(&sp, " ")) {
    }
    ...
    if (!sp.empty()) {
      ...
      is_prev_space = linpipe::sentencepiece::absl::EndsWith(sp, " ");
    }
  ...
  if (spec_->remove_extra_whitespaces()) {
    const linpipe::sentencepiece::absl::string_view space =
...
std::pair<absl::string_view, int> Normalizer::NormalizePrefix(
    linpipe::sentencepiece::absl::string_view input) const {
  ...
  if (trie_ != nullptr) {
    ...
    linpipe::sentencepiece::Darts::DoubleArray::result_pair_type
  ...
  if (longest_length == 0) {
    ...
    if (!string_util::IsValidDecodeUTF8(input, &length)) {
      ...
      result.first = linpipe::sentencepiece::absl::string_view(kReplacementChar);
    } else {
      result.second = length;
      result.first = linpipe::sentencepiece::absl::string_view(input.data(), result.second);
    }
  } else {
    result.second = longest_length;
    // No need to pass the size of normalized sentence,
    // since |normalized| is delimitered by "\0".
    result.first = linpipe::sentencepiece::absl::string_view(&normalized_[longest_value]);
  }
...
std::string Normalizer::EncodePrecompiledCharsMap(
    linpipe::sentencepiece::absl::string_view trie_blob, linpipe::sentencepiece::absl::string_view normalized) {
...
util::Status Normalizer::DecodePrecompiledCharsMap(
    linpipe::sentencepiece::absl::string_view blob, linpipe::sentencepiece::absl::string_view *trie_blob,
    linpipe::sentencepiece::absl::string_view *normalized, std::string *buffer) {
  uint32 trie_blob_size = 0;
  if (blob.size() <= sizeof(trie_blob_size) ||
      !string_util::DecodePOD<uint32>(
          linpipe::sentencepiece::absl::string_view(blob.data(), sizeof(trie_blob_size)),
          &trie_blob_size) ||
  ...
  *trie_blob = linpipe::sentencepiece::absl::string_view(buffer->data(), trie_blob_size);
#else
  *trie_blob = linpipe::sentencepiece::absl::string_view(blob.data(), trie_blob_size);
#endif

  blob.remove_prefix(trie_blob_size);
  *normalized = linpipe::sentencepiece::absl::string_view(blob.data(), blob.size());
...
int PrefixMatcher::PrefixMatch(absl::string_view w, bool *found) const {
  ...
  linpipe::sentencepiece::Darts::DoubleArray::result_pair_type trie_results[kResultSize];
  const int num_nodes =
...
std::string PrefixMatcher::GlobalReplace(absl::string_view w,
                                         linpipe::sentencepiece::absl::string_view out) const {
  std::string result;
  while (!w.empty()) {
    bool found = false;
