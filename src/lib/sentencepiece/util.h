...
const typename Collection::value_type::second_type &FindOrDie(
  ...
  CHECK_MESSAGE(it != collection.end(), "Map key not found: " << key);
  return it->second;
}
...
void InsertOrDie(Collection *const collection,
  ...
  CHECK_MESSAGE(InsertIfNotPresent(collection, key, data), "duplicate key");
}
...
#define CHECK_OR_RETURN(condition)                           \
  if (condition) {                                           \
    ...
    return ::linpipe::sentencepiece::util::StatusBuilder(             \
               ::linpipe::sentencepiece::util::StatusCode::kInternal) \
