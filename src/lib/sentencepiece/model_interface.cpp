...
std::vector<absl::string_view> SplitIntoWords(absl::string_view text,
  ...
  if (treat_ws_as_suffix) {  // put ws tokens at the end of non-ws sequences.
    ...
    while (begin < end) {
      ...
      const bool is_ws = linpipe::sentencepiece::absl::string_view(begin, mblen) == kSpaceSymbol;
      ...
      result.back() =
          linpipe::sentencepiece::absl::string_view(result.back().data(), result.back().size() + mblen);
      begin += mblen;
    ...
    while (begin < end) {
      const int mblen =
          std::min<int>(string_util::OneCharLen(begin), end - begin);
      bool is_ws = linpipe::sentencepiece::absl::string_view(begin, mblen) == kSpaceSymbol;
...
std::string ByteToPiece(unsigned char c) {
  return linpipe::sentencepiece::absl::StrFormat("<0x%02X>", c);
}

int PieceToByte(absl::string_view piece) {
  using PieceToByteMap = linpipe::sentencepiece::absl::flat_hash_map<std::string, unsigned char>;
