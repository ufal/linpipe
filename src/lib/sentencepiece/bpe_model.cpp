...
#include "lib/sentencepiece/freelist.h"
...
std::vector<std::pair<absl::string_view, int>> Model::SampleEncode(
    linpipe::sentencepiece::absl::string_view normalized, float alpha) const {
  ...
  struct Symbol {
    ...
    linpipe::sentencepiece::absl::string_view piece;
  };
  ...
  linpipe::sentencepiece::absl::flat_hash_map<absl::string_view,
                      std::pair<absl::string_view, linpipe::sentencepiece::absl::string_view>>
      rev_merge;
  ...
  auto MaybeAddNewSymbolPair = [this, &symbol_pair_allocator, &symbols, &agenda,
    ...
    if (left == -1 || right == -1 || symbols[left].freeze ||
      ...
      return;
    const linpipe::sentencepiece::absl::string_view piece(
  ...
  while (!normalized.empty()) {
    ...
    s.piece = linpipe::sentencepiece::absl::string_view(normalized.data(), mblen);
  ...
  while (!agenda.empty()) {
    ...
    symbols[top->left].piece = linpipe::sentencepiece::absl::string_view(
    ...
    symbols[top->right].piece = linpipe::sentencepiece::absl::string_view("");
