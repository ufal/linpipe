...
#ifndef SWIG
namespace linpipe::sentencepiece::absl {
...
class Status {
 public:
  Status();
  ~Status();
  Status(StatusCode code, linpipe::sentencepiece::absl::string_view error_message);
...
#define DEFINE_SPP_IMMUTABLE_PROTO_IMPL(FuncName, OutType, ...)      \
  ...
  virtual std::vector<std::vector<std::string>> NBestEncodeAsPieces(
      linpipe::sentencepiece::absl::string_view input, int nbest_size) const {
  ...
  virtual ImmutableSentencePieceText SampleEncodeAsImmutableProto(
      linpipe::sentencepiece::absl::string_view input, int nbest_size, float alpha) const {
