...
#include "lib/mbedtls/include/crypto_oid.h"
...
#include "lib/mbedtls/include/pk_internal.h"

#include <string.h>

#include "lib/mbedtls/include/mbedtls/psa_util.h"
#include "lib/mbedtls/include/psa/crypto.h"

/* Extended formats */
#if defined(MBEDTLS_PEM_PARSE_C)
#include "lib/mbedtls/include/mbedtls/pem.h"
#endif
                         ...
                         const unsigned char *pwd, size_t pwdlen)
{
    ...
    /* If PSA_WANT_KEY_TYPE_RSA_PUBLIC_KEY is defined but PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY isn't,
                                ...
                                const unsigned char *key, size_t keylen)
{
    ...
    if (ret == 0) {
        ...
        if ((ret = mbedtls_pk_rsa_set_pubkey(ctx, p, pem.buflen)) != 0) {
            mbedtls_pk_free(ctx);
        }
    ...
    p = (unsigned char *) key;
    ret = mbedtls_pk_rsa_set_pubkey(ctx, p, keylen);
    if (ret == 0) {
        return ret;
    }
    mbedtls_pk_free(ctx);
